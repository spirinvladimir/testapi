        ...
        maximize: function () {
            if (this.model.get('max') === true) {
                this.model.get('ff').models.forEach(function (fare) {
                    fare.view = new FareView({
                        model: fare
                    });
                    this.el.appendChild(fare.view.el);
                }, this);
                this.model.set('z', 2);
                ...
                this.model.get('ff').models.forEach(function (fare) {
                    this.el.removeChild(fare.view.el);
                }, this);	
...
module.export = {
    ...
    Fares: {reset: function (airline_name, fare_data) {
	var fare = airlines.where({name: airline_name})[0].get('ff').set(fare_data);
    }}
};
